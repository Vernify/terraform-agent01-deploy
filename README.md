...
# Apply (provision agent01 on Proxmox)
terraform apply \
  -var proxmox_password='<proxmox-password>' \
  -var 'ssh_public_keys=["ssh-rsa AAAA... user@host"]'
```
...
3. Create TFC workspace: `agent01`
...
- **vm_name** — VM name (default: `agent01`, do not change)
...
- **ipv4_address** — Static IP CIDR or 'dhcp' (default: `192.168.22.53/24`)
...
- **agent01_vm_id** — Proxmox VM ID (integer)
- **agent01_ipv4_address** — IP address assigned to agent01 (CIDR notation)

## Deployment Flow

This repository is invoked by the Phase 5 orchestration playbook (`bootstrap-container/playbooks/phase-5-orchestrate.yml`):

```yaml
- name: Terraform provision agent01
  terraform:
...
After provisioning:
1. Ansible waits for SSH on agent01 (port 22)
2. Converges Ansible roles:
   - `blueprints.jenkins.agent` (deploy Jenkins agent, connect to build01)
   - `blueprints.vault_integrations.vault_agent` (deploy Vault agent, auto-renew secrets)
   - Install host toolchain: Packer, Terraform, Ansible
3. Verifies agent01 appears in Jenkins UI as healthy node
...
- SSH as `ubuntu` user (not `root`): `ssh ubuntu@192.168.22.53`
- Check cloud-init logs on VM: `cloud-init status`

## Related Repositories

- **terraform-sec01-deploy** — Provisions sec01 (security core)
- **terraform-build01-deploy** — Provisions build01 (CI core)
...
- Vernify Phase 3-5 Architecture Design: `iac-foundry/docs/design/BLUEPRINTS_PHASE_3_5_ARCHITECTURE.md` (Section 4.3)
